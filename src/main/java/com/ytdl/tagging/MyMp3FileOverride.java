...
public class MyMp3FileOverride extends FileWrapper {
	...
	protected MyMp3FileOverride() {
	}

	public MyMp3FileOverride(String filename) throws IOException, UnsupportedTagException, InvalidDataException {
		this(filename, DEFAULT_BUFFER_LENGTH, true);
	}

	public MyMp3FileOverride(String filename, int bufferLength)
			throws IOException, UnsupportedTagException, InvalidDataException {
		this(filename, bufferLength, true);
	}

	public MyMp3FileOverride(String filename, boolean scanFile)
			throws IOException, UnsupportedTagException, InvalidDataException {
		this(filename, DEFAULT_BUFFER_LENGTH, scanFile);
	}

	public MyMp3FileOverride(String filename, int bufferLength, boolean scanFile)
			throws IOException, UnsupportedTagException, InvalidDataException {
		super(filename);
		init(bufferLength, scanFile);
	}

	public MyMp3FileOverride(File file) throws IOException, UnsupportedTagException, InvalidDataException {
		this(file, DEFAULT_BUFFER_LENGTH, true);
	}

	public MyMp3FileOverride(File file, int bufferLength) throws IOException, UnsupportedTagException, InvalidDataException {
		this(file, bufferLength, true);
	}

	public MyMp3FileOverride(File file, int bufferLength, boolean scanFile)
			throws IOException, UnsupportedTagException, InvalidDataException {
		super(file);
		init(bufferLength, scanFile);
	}
	...
	protected int preScanFile(RandomAccessFile file) {
		...
		try {
			file.seek(0);
			int bytesRead = file.read(bytes, 0, HEADER_LENGTH);
			if (bytesRead == HEADER_LENGTH) {
				try {
					ID3v2TagFactory.sanityCheckTag(bytes);
					return HEADER_LENGTH + BufferTools.unpackSynchsafeInteger(bytes[DATA_LENGTH_OFFSET],
							bytes[DATA_LENGTH_OFFSET + 1], bytes[DATA_LENGTH_OFFSET + 2],
		...
		if (bytes.length >= offset + XING_MARKER_OFFSET_1 + 3) {
			...
			if (bytes.length >= offset + XING_MARKER_OFFSET_2 + 3) {
				...
				if (bytes.length >= offset + XING_MARKER_OFFSET_3 + 3) {
					if ("Xing".equals(BufferTools.byteBufferToStringIgnoringEncodingIssues(bytes,
							offset + XING_MARKER_OFFSET_3, 4)))
						return true;
					if ("Info".equals(BufferTools.byteBufferToStringIgnoringEncodingIssues(bytes,
							offset + XING_MARKER_OFFSET_3, 4)))
						return true;
				}
			}
